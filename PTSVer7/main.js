...
const CONFIG = {
  ...
  // ------------------------------------------------------------ curve strands
  // The three curves from Corner_1.fbx, drawn rather than generated. Everything the walked
  // rays needed — a heading, a way to stop them wandering off, a bias to keep them apart —
  // is answered by the fact that someone drew them where they wanted them.
  curveMotes: 850,         // motes on each curve
  curveJitter: 0.011,       // lateral scatter about the line, in plane widths — the
                            //   thickness of the rope
  curveTravel: 0.16,        // how far along its curve a mote slides over one life, as a
                            //   fraction of the whole curve. This is the streaming, and it
                            //   runs ALONG the line for the same reason the model's threads
                            //   do: measured on Ref1, 97.8% of the motion follows the
                            //   filament rather than crossing it
  // The curves were drawn centred on the model, and the model is centred on the screen
  // corner — so three quarters of a curve drawn that way is off-screen past the corner, and
  // what is left reads as a scribble in the corner rather than as the line someone drew.
  // Fitted into the visible quadrant instead: same drawing, same proportions, placed where
  // it can be seen. 0 turns the fit off and leaves them exactly where the file puts them.
  curveFit: 1,
  curveFitMargin: 0.10,     // breathing room inside the quadrant, as a fraction of it

  curveSize: 1.4,           // drawn heavier than the mass around them, and exempt from the
                            //   edge shrink: they are deliberate features, not part of the
                            //   fade
...
uniform float uCurveTravel;      // how far along its curve one life carries a mote, world
...
attribute float aCurveId;        // which curve, or -1 for the mass's own motes
attribute vec3  aCurveDir;       // the curve's direction where the mote was seated
...
void main(){
  ...
  vec3 pos;
  if (aCurveId >= 0.0) {
    // A mote on a curve slides ALONG it, down the tangent it was seated on. The tangent is
    // baked per mote rather than the curve being read here: reading the line itself would
    // mean a texture fetch in the vertex stage, and the travel over one life is short
    // enough against the curve's own bends that the straight version is the same line.
    pos = aInitPos + aCurveDir * (uCurveTravel * lifePhase * aLife);
  } else {
    pos = advect(aInitPos, uLifeDrift * lifePhase * aLife, aOutward * uOutward);
  }
...
// ---------------------------------------------------------------- the authored curves
// Corner_1.fbx: three open NURBS curves, resampled evenly along their arc length and
// quantised the same way the mesh is. They arrive in the mesh's own frame — centred on
// Corner.fbx's bounding box rather than their own — so they sit where they were drawn
// relative to the mass, and they take the same rotation and fit as the model.
//
// Their control points are used directly as a polyline. At 337 points over a curve this
// smooth, the control polygon and the spline it describes are the same line to well under
// the size of a mote, and evaluating the basis would buy nothing but code.
const CURVES_B64 =
  'QIgMwNK8Kr8NHqu/D2uGQJ7Aqj8HdSNAAwAAAZzA///JaUy/Gf6CaPy9M/w/Z5+8WPocZkC7fPj+ZNm5ofYAZHC4xfQHYwK3' +
  '3/IsYpO19vBWYSG0+O6cYK+y9ezmXz2x0OpKX82vo+ixXmGuRuYsXves2+OqXZerNeEyXT2qb965XPGobts9XLinL9irW46m' +
  'x9QHW5SlA9EgWsykGM35WESkIcmKVwqkQsXMVT+k88HDU6GkCr+oUS6lfbx/T+Slb7pUTaemibgpS36n7rYDSWKohLXgRkep' +
  'G7S9RD+qCbOkQjir+7GLQDCs7LByPjmtD7BnPESuPq9gOlCvbK5YOGCwo61VNn6x/KxlNJyyVKx0MrqzrKuDMNy0DKuZLgq2' +
  'gqrALDi396nnKme4bKkOKZe55ag5J9O6bqh5JRC896e5I029gaf5IYm+Cqc5INO/oaaSHhzBOKbrHGbCz6VEG7DDZ6WdGQTF' +
  'hKHytV20TqHftmCzF6HLt2Oy4aC4uGWxrqChuWKwi6B7ul6vaKBVu1uuRaAuvFetIqAIvVSs/5/ivUyr7Z+vvkOq4Z93vzqp' +
  '1Z8/wDKoyJ8HwSmnvJ/QwSCmsJ+YwhilqZ9jww+kop8txAejm5/4xP+hlJ/DxfegjZ+NxvGfgZ9gx+2ecp83yOidY58PyeSc' +
  'VJ/nyeCbRJ+/ytuaNZ+Xy9qZHJ94zNqYAZ9czdqX5p5AztmWy54kz9mVr54I0NmUlJ7s0NuTa57Z0d6SQJ7G0uGRFp6z0+WQ' +
  '652h1OiPwJ2O1euOlZ181vCNWZ1t1/WMG51f2PqL3ZxR2f+Kn5xE2gSKYZw22wmJI5wo3A6I0psZ3RSHfpsJ3hmGKpv63h6F' +
  '1prq3yOEgpra4CiDLprL4SyCx5mz4i6BXJma4zGA8JiA5DN/hJhm5TZ+GJhM5jh9rZcz5zd8NJcL6DV7tpbf6DJ6OJaz6TB5' +
  'upWH6i54PJVb6yt3vpQv7CV2M5T07B11o5Oy7RZ0E5Nw7g5zg5Iu7wZy85Ht7/9wY5Gr8PRvwJBX8ehuF5D+8dxtb4+m8tBs' +
  'x45N88RrH47087dqco2W9Kdps4wl9Zho9Yu09YhnN4tD9nlmeYrS9mlluYlg91dk6IjZ90RjFohR+DJiRIfJ+CBhc4ZB+Q1g' +
  'noW2+flevIQb+uZd2YOA+tJc94Lk+r5bFIJJ+6paKIGk+5VZNoD6+4FYRX9Q/G1XU36m/FlWXH34/ERVW3xF/TBUW3uR/RxT' +
  'Wnre/QlSUnkp/vZQQnhy/uNPMne7/tBOIXYE/w==';
// Unpack to a flat sample array: curveCount x samplesPerCurve positions.
function decodeCurves(b64) {
  ...
  const lo = [dv.getFloat32(0, true), dv.getFloat32(4, true), dv.getFloat32(8, true)];
  const span = [dv.getFloat32(12, true), dv.getFloat32(16, true), dv.getFloat32(20, true)];
  const nCurves = dv.getUint16(24, true);
  const nSamples = dv.getUint16(26, true);
  const q = new Uint16Array(buf, 28, nCurves * nSamples * 3);
  const pos = new Float32Array(nCurves * nSamples * 3);
  for (let i = 0; i < nCurves * nSamples; i++) {
    for (let a = 0; a < 3; a++) pos[i * 3 + a] = lo[a] + (q[i * 3 + a] / 65535) * span[a];
  }
  return { pos, nCurves, nSamples };
}
...
function rasteriseMaps(res) {
  ...
  return { depth, normZ, res, aspect: spanX / spanY, spanX };
}
...
function buildParticles(count) {
  ...
  const curveDir   = new Float32Array(count * 3);
  const curveIdA   = new Float32Array(count).fill(-1);
  ...
  // ---- the authored curves, placed in the world the mass lives in ----------
  // Same rotation as the model and the same scale its own width was fitted by, so a curve
  // drawn against the shape lands against the shape. This is the entire reason the bake
  // centres them on the MESH's bounding box rather than their own.
  const curves = decodeCurves(CURVES_B64);
  const curveScale = planeW / Math.max(1e-6, maps.spanX);
  {
    const rot = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(
      THREE.MathUtils.degToRad(CONFIG.modelRotX),
      THREE.MathUtils.degToRad(CONFIG.modelRotY),
    ...
    const v = new THREE.Vector3();
    for (let i = 0; i < curves.pos.length; i += 3) {
      v.set(curves.pos[i], curves.pos[i + 1], curves.pos[i + 2])
        .applyMatrix4(rot).multiplyScalar(curveScale);
      curves.pos[i] = v.x; curves.pos[i + 1] = v.y; curves.pos[i + 2] = v.z;
    }
  }
  // Fit the whole curve SET — not each curve — into the visible quadrant, keeping its
  // proportions, so the three keep their relationship to one another.
  if (CONFIG.curveFit) {
    let lo = [Infinity, Infinity, Infinity], hi = [-Infinity, -Infinity, -Infinity];
    for (let i = 0; i < curves.pos.length; i += 3) {
      for (let a = 0; a < 3; a++) {
        lo[a] = Math.min(lo[a], curves.pos[i + a]);
        hi[a] = Math.max(hi[a], curves.pos[i + a]);
      }
    }
    const m = 1 - CONFIG.curveFitMargin;
    const boxW = planeW * 0.5 * m, boxH = planeH * 0.5 * m;
    const k = Math.min(boxW / Math.max(1e-6, hi[0] - lo[0]),
                       boxH / Math.max(1e-6, hi[1] - lo[1]));
    // the quadrant on screen is the negative one, so its centre is a quarter of the way
    // down and in from the corner
    const cx = -planeW * 0.25, cy = -planeH * 0.25;
    const mid = [(lo[0] + hi[0]) / 2, (lo[1] + hi[1]) / 2, (lo[2] + hi[2]) / 2];
    for (let i = 0; i < curves.pos.length; i += 3) {
      curves.pos[i]     = (curves.pos[i] - mid[0]) * k + cx;
      curves.pos[i + 1] = (curves.pos[i + 1] - mid[1]) * k + cy;
      curves.pos[i + 2] = (curves.pos[i + 2] - mid[2]) * k;
    }
  }

  const curveSample = (c, t) => {
    // nearest sample is enough on the CPU: this only seats the mote, and the shader
    // interpolates properly when it moves it
    const i = Math.min(curves.nSamples - 1, Math.max(0, Math.round(t * (curves.nSamples - 1))));
    const o = (c * curves.nSamples + i) * 3;
    return [curves.pos[o], curves.pos[o + 1], curves.pos[o + 2]];
  };
  ...
  const extraCount = Math.min(count, curves.nCurves * Math.max(0, CONFIG.curveMotes));
  let cornerIndex = 0;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const curveId = isCorner ? i % curves.nCurves : -1;
    ...
    if (isCorner) {
      // Seated at a random point ALONG its curve, spread evenly over the whole length —
      // which is what makes the line continuous while every mote on it is at a different
      // point of its own life. The parameter travels with the mote; the shader reads the
      // curve at t plus however far the mote has slid.
      const t = Math.random();
      const at = curveSample(curveId, t);
      // the line's own direction here, taken from the next sample along
      const nxt = curveSample(curveId, Math.min(1, t + 1 / (curves.nSamples - 1)));
      let dx = nxt[0] - at[0], dy = nxt[1] - at[1], dz = nxt[2] - at[2];
      const dl = Math.hypot(dx, dy, dz) || 1;
      curveDir[i3] = dx / dl; curveDir[i3 + 1] = dy / dl; curveDir[i3 + 2] = dz / dl;
      const j = CONFIG.curveJitter * planeW;
      px = at[0] + gauss1() * j;
      py = at[1] + gauss1() * j;
      pz = at[2] + gauss1() * j;
      nz01 = 1;
    ...
    if (isCorner) mult *= CONFIG.curveSize;
  ...
  inst('aCurveId', curveIdA, 1);
  inst('aCurveDir', curveDir, 3);
  ...
  geo.userData.src = {
    ...
    aCurveId: curveIdA.slice(), aCurveDir: curveDir.slice(),
  };
...
const uniforms = {
  ...
  uCurveTravel: { value: 0 },        // set in place(), where the plane's world size is known
...
function place() {
  ...
  uniforms.uCurveTravel.value = CONFIG.curveTravel * seatPlaneW;
