...
const METAL = {
  ...
  tint: 0xd42a10,        // metal body color — red (0xffffff = neutral chrome)
...
// rasterized onto plate UV as pattern-mask.webp (white = inside the pattern).
...
// normal.webp's actual slope/edge signal (the real geometry's groove outline) by
...
// That correction is baked directly INTO pattern-mask.webp (a one-time affine
...
uniform vec3 uMetalTint;         // metal body color (red)
uniform float uScreenScroll;
...
void main(){
  ...
  vec3 metalBase = color * uMetalTint;                            // tinted metal body
...
const tPatternMask = texLoader.load('./pattern-mask.webp', () => { uPatternMaskLoaded.value = 1; });
...
function makeWallMaterial(bake1, bake2) {
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uChromaColor: { value: new THREE.Color().setHex(CHROMA.color, THREE.LinearSRGBColorSpace) },
