...
const CONFIG = {
  ...
  cornerFull: false,        // AURORA: throw the seeds the whole way round instead of a
                            //   quarter-disc. ver15: OFF - see cornerHalf
  ...
  // the same calm pace.
  simSpeed: 0.066,          // AURORA ver11: back to ver10's pace — the customer asked for
  ...
  flightDur: 1.10,          // seconds a mote takes to cross, on average (0.77 - 1.43)
  flightSpread: 1.00,       // seconds of random delay across the population, so it leaves as
                            //   a stream rather than a block. Longest arrival: 1.00 + 1.43 =
                            //   2.43s, inside the two and a half asked for. The spread is the
  ...
  flightNoise: 0.20,        // ver15: how far a mote WANDERS off its route, as a fraction of
...
vec2 flightOwed(float seedw, float now, float birth){
  ...
  float ph1   = fract(seedw * 13.31) * 6.2831853;
  float ph2   = fract(seedw * 71.17) * 6.2831853;
  float f1    = mix(0.8, 2.2, fract(seedw * 37.73));
  float f2    = mix(2.0, 4.0, fract(seedw * 19.91));
  float fa    = mix(0.6, 1.6, fract(seedw * 47.29));
  float wr    = fract(seedw * 83.41);
  float wamp  = uFlightNoise * (0.35 + 1.9 * wr * wr);
  vec2 owed = vec2(0.0);
  for (int k = 0; k < 8; k++) {
    vec2 D = uFlightD[k];
    ...
    float env = sin(3.14159265 * e);   // zero at both ends, so the endpoints stay exact
    float across = env * (sin(6.2831853 * f1 * e + ph1) + 0.5 * sin(6.2831853 * f2 * e + ph2));
    float along  = env * sin(6.2831853 * fa * e + ph2);
    owed += D * (1.0 - e)
          - perp * (env * uFlightArc * side + across * wamp)
          - D * (along * wamp * 0.35);
  }
  return owed;
}
