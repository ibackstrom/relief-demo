...
const CHROMATIC = {
  ...
  //
  // Given as a number on the 0..1 wheel, never as a hex picked by eye: a colour chosen
  // visually lands elsewhere. The shader takes fract() of it, so 1.0 is the top of the
  // wheel and identical to 0.0 — pure red, with nothing of the magenta side left in it.
  hue: 1.0,
  ...
  //
  // A band moves as a whole — its two ends hold their ratio. Scaling it down is the
  // control for "less colour without less reach": at 0 the sheen is plain grey.
  //
  // satRange runs PAST 1 deliberately. hsv2rgb ends on mix(white, pureHue, S) and does
  // not clamp S, so above 1 the channels the hue does not use go negative and subtract
  // the wall's own green and blue where the colour lands. That is what makes the red
  // deeper than a pure hue can be, and it is why valRange can sit this low without the
  // sheen going muddy: the darkness comes from V, the bite from S over 1.
  satRange: [0.863, 1.570],
  valRange: [0.342, 0.650],
...
const OPACITY_SIZE = ['s', 'm', 'l'].includes(PARAMS.get('op')) ? PARAMS.get('op') : 'l';
...
// The hue in force. Only the hue is set here; S and V come from satRange/valRange.
const chromaHue = () => MASK.hue;
...
const CUSTOM = {
  model: '../shared/bakes/model.glb',   // welded copy written by scripts/bake_levels.py — NOT output.gltf
  bake1: '../shared/bakes/bake1.webp',
  bake2: '../shared/bakes/bake2.webp',
  meta: '../shared/bakes/meta.json',    // depthMult etc. — written by the bake, no manual sync
...
// The query switches below stay live for a further pass, but the page names none of
// them: the title and the hint are index.html's, and what ships is the build's own
// settings. Comparison runs identify themselves by their switches in the URL.
...
// again.
//
// OFF by default — the values it was used to find are in CHROMATIC above, and the shipped
// page carries no controls. ?ui=1 brings it back for another pass.
...
if (uiEl && PARAMS.get('ui') !== '1') {
  ...
  const ROWS = [
    { label: 'hue', cst: 'CHROMATIC.hue', uni: 'uChromaHue',
      value: chromaHue(), step: 0.001 },
    // Saturation runs to 4, well past the 1 where HSV saturation nominally stops.
    //
    // hsv2rgb ends on mix(white, pureHue, S), and above S = 1 that keeps going rather
    // than clamping: the channels the hue does not use go NEGATIVE, and since the result
    // is then blended into the wall, they subtract the wall's own green and blue. So past
    // 1 the control stops being "how pure is the colour" and becomes "how much green and
    // blue is taken out of the wall underneath" — a deeper red than a pure hue can be.
    //
    // It runs out when the blended green reaches 0, which at this opacity is around
    // S = 7; 4 is past anything that still reads as a sheen rather than a solid patch.
    // Values above 1 are still valid to write into CHROMATIC — the shader does this
    // arithmetic either way, it is only HSV's definition that stops at 1.
    { label: 'saturation', cst: 'CHROMATIC.satRange', uni: 'uChromaSatRange',
      band: MASK.satRange, value: MASK.satRange[1], step: 0.01, max: 4 },
  ...
  uiEl.innerHTML = '<h2>red mask colour</h2>' + ROWS.map((r, i) =>
    ...
    + '<input type="range" id="uir' + i + '" min="0" max="' + (r.max || 1) + '"'
    + ' step="' + r.step + '" value="' + r.value + '"></div>'
  ).join('') + '<div class="foot"><button id="uireset">reset</button> &nbsp; drop ?ui=1 to hide</div>';
