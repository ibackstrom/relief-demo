# PTSVer30 — corner particle cloud, under a label

ver30 splits the population in two **without adding any**. Half the motes belong to the text:
they are born on the label's box, they stay there, they ignore the pointer, and they carry a
little more ink. The other half drift, bloom and answer the cursor exactly as ver29's do.
`?seats=0` is ver29 unchanged. See [Two groups, one system](#two-groups-one-system).

ver29 is ver28 at the placement that was settled on its panel — `offsetX` **0.220**, `offsetY`
**0.245** — **and with no tuning rail**. `index.html` no longer carries the `#pui` element, so
main.js finds nothing to build the panel into and `?ui=1` has nothing to show either. `?x=` and
`?y=` still take the placement, which is how to try another one without a build.

It is otherwise ver18: the client's hover settings, the pull toward the sign, and none of
ver19-27's leash, chunk, squash or extra bars.
...
`?ui=1` shows it. Nine bars in ver18 — the seven hover ones, plus the two for the sign's pull,
which is new and has to be tuned by eye. Everything the cloud does on its own is settled and baked, and a bar for
...
corner falls into. `attractCore` is 0.30 of the reach, `attractRadius` 0.34 of the viewport
height, `attractPull` 0.10.
...
## Two groups, one system

**Neither group is a second system.** One geometry, one simulation texture, one draw call, one
pair of shaders. A mote belongs to the text's group if its *seat* falls inside the label's box,
and that is settled once when the seats are thrown — the test is a box distance against a
constant, so it costs a few instructions per mote per frame against a two-octave curl field and
a per-pixel lit sphere. **The particle count is unchanged, so there is nothing extra for a
phone to draw.**

What the text's group does differently:

- **Born on the label**, evenly across its box — a rectangle, not an ellipse, because an
  ellipse inscribed in a wide short box narrows to a point at exactly the place the last
  letters are.
- **Stays there.** `signLeash` 0.10 mass radii from its own seat, applied to the position
  rather than the velocity, so a mote at the limit keeps its momentum and slides along the
  boundary instead of stopping dead. A dead one respawns on the label, because that is where
  its seat is.
- **Ignores the pointer.** `signShield` 0, and it is held out of the bloom as well — the bloom
  scales about the screen corner, so on hover it would otherwise carry the backing off the
  words at the moment somebody is looking at them.
- **Carries more ink.** `signInk` 0.30. The cloud is faint by design at `alphaGain` 0.43,
  which is right for a dusting over a wall and not enough for white type to be read against.

**The cost is in the look, not in the machine.** Every mote given to the text is one the free
cloud no longer has, so at half and half the drifting part is half as dense as ver29's. That is
the trade to judge on the link — `?seats=0.25` and `?seats=0.1` are the same build with a
smaller share given away.
