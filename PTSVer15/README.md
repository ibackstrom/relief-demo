# PTSVer15 — corner particle cloud, under a label
...
`1 / (1 - drag * (1 - settle))`, which is about 19 at the heavy end and 1 at the light one.
...
The force is laid along the path the pointer took, as up to eight stamps dropped as the
cursor travels, each fading on its own clock over `hoverTrail` seconds. That is what makes
