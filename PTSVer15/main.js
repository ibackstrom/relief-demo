...
const CONFIG = {
  ...
  hoverPush: 0.25,          // of the mass radius per second
  ...
  hoverTrail: 1.8,          // seconds a stamp keeps acting: the reach BACKWARDS IN TIME.
...
uniform vec4  uStampO[8];
uniform vec3  uStampD[8];
...
// How much of the wall's magnification the cloud takes. 1 is the wall's own, which is what
// pins it; 0 leaves it fixed on the screen, which is what it did before.
const ZOOM_PIN = numParam('pin', 0, 1) !== null ? numParam('pin', 0, 1) : 1;
...
if (uiEl && PARAMS.get('ui') !== '1') {
  ...
  const ROWS = [
    ...
    { key: 'hoverTrail', name: 'memory', cst: 'CONFIG.hoverTrail',
      min: 0, max: 6, step: 0.05, value: CONFIG.hoverTrail,
