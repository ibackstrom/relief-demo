// immersive-g.com home bas-relief reveal — ver11.
// ver10 at ?red=0.35 — the tight red core the customer settled on — plus the one thing
// it could not do: put colour on the BACKDROP as well as on the contours. Every mask
// term up to here (rim, crease) keys off the relief's own lines and shadows, so the flat
// wall between them stayed bare at any setting. CHROMATIC.base is a floor under the
// revealed patch, which is the only term that touches the wall itself.
// Painted to the customer's own sketch (_shots/customer-sketch.jpg) at the half strength
// they asked for, and offered in three sizes — see ?bg.
...
const CHROMATIC = {
  ...
  base: 0.22,
  // How far out the wash reaches, as a band on extrude: the relief is deepest under the
  // cursor and shallows toward the edge of the reveal, so gating on depth gates on
  // distance from the cursor without needing a second radius. See ?bg below — this is
  // the value the three sizes replace.
  baseGate: [0.12, 0.50],
  // The dye density at which the wash reaches full strength, in raw dye units. This is
  // the softness control: the mask's own ramp saturates at 6.7 and the splat lays down
  // ~9 a frame, so anything near that reads the dye as a flat disc and paints a slab.
  // 25 is about the density the trail's core actually reaches at a normal cursor speed,
  // so the haze arrives at full strength under the cursor and gradates away from there,
  // and fades out behind as the dye dissipates. Lower = denser and harder-edged (at the
  // mask's own 6.7 it is a flat slab), higher = the haze never reaches full strength: 50
  // measured a peak of 13 against the sketch's target of 24.
  baseFull: 25,
...
//
//   ?bg=s   the haze sits in the deepest part of the reveal only, right under the cursor
//   ?bg=m   the default
//   ?bg=l   it fills the revealed patch out to its edge
//   ?bg=0   no wash at all — ver10
//
// Only the reach changes. CHROMATIC.base is the same in all three, so the haze is the
// same colour at its densest and the sizes differ in how far that density carries.
//
// The size is a RADIUS — baseSpread, how far the dye is dilated before the wash reads it,
// in screen uv. The first attempt sized these by the extrude gate alone and the three
// came out at 6.2 / 8.3 / 10.6% of frame, far too close to choose between: the wash was
// riding the undilated dye, whose disc at ?red=0.35 is small and saturated nearly to its
// own edge, so no threshold on it carves a meaningfully different circle. Same trap as
// ver8's first set of feathers.
//
// Everything else is shared, so the three differ in reach and nothing else: the same
// baseGate on extrude keeps the haze on relief the cursor has actually raised, and the
// same baseFull gives it the same softness. A slow cursor extrudes less and lays down
// less dye, so the haze is smaller when moving slowly — the same depth-follows-speed
// behaviour the whole reveal has.
const BG = {
  s: { baseSpread: 0.000 },
  m: { baseSpread: 0.022 },
  l: { baseSpread: 0.055 },
...
uniform float uChromaBaseSpread;
...
void main(){
  ...
  // OURS (ver11). The background wash: colour on the flat wall between the lines, which
  // no other term can put there. Two gates, and they do different jobs —
  //   extrude   keeps it on relief the cursor has actually brought out, so the haze
  //             never appears on a bare wall;
  //   the dye   keeps it under the cursor, and its REACH is the ?bg size. uChromaBaseSpread
  //             dilates the dye first (ring max, in uv), which is how the haze can carry
  //             past the red on the contours instead of stopping where the dye stops.
  // Dilation is safe here where it was not for the crease: the dye is a smooth blob, so
  // a ring of taps blurs it, while on the relief's thin strokes the same ring left a comb
  // of offset copies (see deepShadingAt).
  float washDye = fluid.b;
  if (uChromaBaseSpread > 0.0001) {
    for (int i = 0; i < 6; i++) {
      float a = float(i) * 1.0471975 + 0.3;       // 6 taps, 60 degrees apart
      vec2 off = vec2(cos(a), sin(a)) * uChromaBaseSpread;
      off.x /= uResolution.x / uResolution.y;     // a circle on screen, not on uv
      washDye = max(washDye, texture2D(tFluidFlowmap, uvScreen + off).b);
    }
  }
  // The dye is read SOFTLY here, against uChromaBaseFull rather than through the mask's
  // own fluidMagnitude ramp. That ramp saturates at density 6.7 and the splat lays down
  // ~9 per frame, so by the mask's reckoning the dye is a flat-topped disc and any
  // threshold on it produces a hard-edged slab — which is exactly what the first build
  // of this wash looked like: a painted pink bar, not the customer's soft haze. Reading
  // it against a value well above saturation puts the wash back on the dye's own profile,
  // which falls off with distance from the cursor and decays with age behind it.
...
function makeWallMaterial(bake1, bake2) {
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uChromaBaseSpread: { value: MASK.baseSpread || 0 },
...
if (!SITE_MASK) {
  ...
  document.title = 'ver11 — background wash: ' + bgName
