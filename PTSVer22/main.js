...
const CONFIG = {
  ...
  signBiasX: 1.30,          // how far to shift the sign's anchor toward the END of the words,
  ...
  signSeats: 0.05,          // share of the population seated under the label. At 450k that is
  ...
  signTrap: 0.0,            // THE TRAP, and it is what actually keeps the words backed.
...
void main(){
  ...
  float under = 1.0 - smoothstep(0.65, 1.0, length(q));
...
void main(){
  ...
  float held = max(caught, (1.0 - smoothstep(0.65, 1.0, length(seatQ))) * uSignHold);
...
uniform vec3  uSignPoint;        // the label's centre and box, for the patch held under it
...
void main(){
  ...
  float underSign = (1.0 - smoothstep(0.65, 1.0, length(signQ))) * uSignHold;
...
function buildParticles(count) {
  ...
  if (signCount > 0) {
    ...
    if (el) {
      ...
      const lx = ((r.left + r.width / 2 - innerWidth / 2) / ppw
