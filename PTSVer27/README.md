# PTSVer27 — corner particle cloud, under a label
...
ver27 is ver26 with **the hover opened up on the panel again** — nine more bars covering how
the cursor triggers the response and what shape it takes, on top of the seven that were already
there. See [The panel](#the-panel). Nothing else changed.

ver26 lifts it further: `flowAngle` **171**, which is nine degrees ABOVE level rather than
level. Past 180 the throw runs left and up, so the far end now rises away from the corner
instead of merely stopping short of drooping.
...
of why it hung there. `flowAngle` was **182** in ver25, all but level, so the same mass reaches across
...
| bloom | `expandAmount` | how much the whole volume grows while the pointer is near | `?bloom=` |
| trigger reach | `expandHoverRadius` | how close the pointer must be for the bloom to start | `?trig=` |
| trigger plateau | `expandHoverInner` | inside this it is at full strength — the corner cloud needs it, because its centre *is* the corner and the pointer can never reach it | `?plateau=` |
| bloom ease | `expandSeconds` | how long the growth takes, in and out | `?ease=` |
| arrive/leave | `mouseFadeSeconds` | the ease on the whole response as the pointer arrives and goes | `?arrive=` |
| swirl | `hoverSwirl` | how much of the push turns instead of spreading | `?swirl=` |
| falloff | `falloffPower` | the curve of the push against distance | `?falloff=` |
| edge blur | `mouseEdgeBlur` | how wide a band the reach's boundary is spread over | `?blur=` |
| rim noise | `mouseNoise` | how ragged the opening's rim is | `?rim=` |
