...
const CONFIG = {
  ...
  focusDensity: 0.75,       // ver8 raises this again: the label wants a clear peak under
                            //   it and a proportional fall away from it. Too high and the
                            //   drawing vanishes under a single hot spot.
                            //   0.5 -> 0.75 in ver25, with the taper above: a power on the
                            //   radius of every throw, so it gathers the whole population
                            //   toward the middle rather than shortening one end. The two
                            //   together are "concentrate it": one takes the straggle in, the
                            //   other weights what is left toward the centre
  ...
  flowAngle: 171,           // degrees, the throw axis: which way the pigment was dragged.
                            //   196 ran left and slightly DOWN from the corner, which is why
                            //   the far end of the streak hung below the body — the tail was
                            //   simply pointed there. Past 180 it runs left and UP: 171 lifts
                            //   the far end nine degrees above level, where 182 only stopped
  ...
  streakTaper: 0.62,        // how fast the trailing end thins out, in plane widths. Small
                            ...
                            //   and slightly down, so the trailing end IS the lower left —
                            //   the part that was hanging below the body. Thinning it sooner
                            //   draws that end in without moving the mass or touching the
                            //   dense end under the label
  ...
  attractRadius: 0.55,      // reach, in viewport heights. Raised with the pull above, and it
                            ...
                            //   left: the grip falls off as a gaussian, so at twice the reach
                            //   it is a fiftieth of nothing and a stray never comes back. The
                            //   reach has to cover where they get to, not where they sit.
                            //   Beyond about this the pull is
  ...
  // ------------------------------------------------------------ the leash
  // How far a mote may be carried from its OWN SEAT. The cursor's push is strong and the
  // cloud now remembers it for seconds, so a few motes at the edge get thrown and simply keep
  // going — the field they land in is no stronger than the momentum they arrived with, and
  // nothing was bringing them back.
  //
  // Bounded against the seat rather than against the cloud's centre on purpose: the seats
  // ARE the shape, so this limits how far a mote strays from where it belongs without
  // squeezing the mass into a ball.
  leashRadius: 0.60,        // mass radii. The distance at which the ceiling starts to bite
  leashSoft: 0.30,          // and how much further it can still creep, as a fraction of the
                            //   leash. The excess is compressed asymptotically rather than
                            //   cut, so nothing stops dead at a radius and no sphere appears
                            //   in the picture: the reach is leashRadius * (1 + leashSoft)
                            //   and a mote only ever approaches it

  // ------------------------------------------------------------ the sign's own patch
  // The label is white type on a light wall: it is only legible where there is ink behind it.
  // The pull above biases the drift toward the sign, which is not the same as guaranteeing
  // the words are backed — a bias can still be beaten by a hard sweep of the cursor or by the
  // field, and when it is, the sign disappears.
  //
  // So the motes whose SEATS fall under the words are kept on a much shorter leash than the
  // rest. They may stir, they may be pushed, they cannot leave; and because a dead mote
  // respawns at its seat, the patch behind the sign is continuously refed. It is a guarantee
  // rather than a tendency, and it costs nothing at run time: the test is the seat's distance
  // to the label, which never changes.
  //
...
// the bloom's trigger, all live
if (numParam('bloom', 0, 1.5) !== null) CONFIG.expandAmount = numParam('bloom', 0, 1.5);
if (numParam('trig', 0.02, 0.6) !== null) CONFIG.expandHoverRadius = numParam('trig', 0.02, 0.6);
if (numParam('plateau', 0, 0.4) !== null) CONFIG.expandHoverInner = numParam('plateau', 0, 0.4);
if (numParam('ease', 0.05, 6) !== null) CONFIG.expandSeconds = numParam('ease', 0.05, 6);
if (numParam('arrive', 0, 3) !== null) CONFIG.mouseFadeSeconds = numParam('arrive', 0, 3);
if (numParam('falloff', 0.5, 6) !== null) CONFIG.falloffPower = numParam('falloff', 0.5, 6);
if (numParam('rim', 0, 1.2) !== null) CONFIG.mouseNoise = numParam('rim', 0, 1.2);
...
uniform float uDrag;
uniform float uLeash;            // furthest a mote may be carried from its seat, world units
uniform float uLeashSoft;
...
void main(){
  ...
  // The leash, applied to the POSITION rather than to the velocity. A mote that reaches it
  // is not stopped — it keeps its momentum and slides along the boundary, which reads as the
  // mass holding its edge rather than as particles hitting a wall. Compressed rather than
...
void main(){
  ...
  // The patch under the sign does not bloom with the rest, and that is the other half of
...
function makeSim() {
  ...
  const step = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uLeash: { value: CONFIG.leashRadius * d.radius },
      uLeashSoft: { value: CONFIG.leashSoft },
      uSignHalf: { value: new THREE.Vector2(1e-5, 1e-5) },
      uSignHold: { value: CONFIG.signHold },
...
function stepSim(dt) {
  ...
  u.uLeash.value = CONFIG.leashRadius * sim.radius;
...
if (uiEl && PARAMS.get('ui') !== '1') {
  ...
  const ROWS = [
    ...
    { key: 'attractRadius', name: 'attract reach', cst: 'CONFIG.attractRadius',
      min: 0.02, max: 1.2, step: 0.01, value: CONFIG.attractRadius,
      place: true, text: () => CONFIG.attractRadius.toFixed(2) },
    // ver19: how far a mote may stray from its seat, in mass radii
    ...
    // ---- how the cursor TRIGGERS the response ------------------------------------------
    // The push and the bloom are two different answers to the same pointer, and these are
    // the bloom's: when it starts, how big it gets and how long it takes. Every one is read
    // out of CONFIG each frame, so all of them drag live.
    { key: 'expandAmount', name: 'bloom', cst: 'CONFIG.expandAmount',
      min: 0, max: 1.5, step: 0.01, value: CONFIG.expandAmount,
      uni: 'uExpandAmount', text: () => CONFIG.expandAmount.toFixed(2) },
    // the pointer has to be this close to the cloud's centre before the bloom begins, and
    // inside the plateau it is at full strength — the corner cloud needs the plateau because
    // its centre IS the corner and the pointer can never get near it
    { key: 'expandHoverRadius', name: 'trigger reach', cst: 'CONFIG.expandHoverRadius',
      min: 0.02, max: 0.6, step: 0.005, value: CONFIG.expandHoverRadius,
      place: true, text: () => CONFIG.expandHoverRadius.toFixed(3) },
    { key: 'expandHoverInner', name: 'trigger plateau', cst: 'CONFIG.expandHoverInner',
      min: 0, max: 0.4, step: 0.005, value: CONFIG.expandHoverInner,
      place: true, text: () => CONFIG.expandHoverInner.toFixed(3) },
    { key: 'expandSeconds', name: 'bloom ease', cst: 'CONFIG.expandSeconds',
      min: 0.05, max: 6, step: 0.05, value: CONFIG.expandSeconds,
      text: () => CONFIG.expandSeconds.toFixed(2) + ' s' },
    { key: 'mouseFadeSeconds', name: 'arrive/leave', cst: 'CONFIG.mouseFadeSeconds',
      min: 0, max: 3, step: 0.05, value: CONFIG.mouseFadeSeconds,
      text: () => CONFIG.mouseFadeSeconds.toFixed(2) + ' s' },
    // ---- and the shape of the push itself ----------------------------------------------
    { key: 'hoverSwirl', name: 'swirl', cst: 'CONFIG.hoverSwirl',
      min: 0, max: 1, step: 0.01, value: CONFIG.hoverSwirl,
      text: () => CONFIG.hoverSwirl.toFixed(2) },
    { key: 'falloffPower', name: 'falloff', cst: 'CONFIG.falloffPower',
      min: 0.5, max: 6, step: 0.1, value: CONFIG.falloffPower,
      text: () => CONFIG.falloffPower.toFixed(1) },
    { key: 'mouseEdgeBlur', name: 'edge blur', cst: 'CONFIG.mouseEdgeBlur',
      min: 0, max: 1, step: 0.01, value: CONFIG.mouseEdgeBlur,
      text: () => CONFIG.mouseEdgeBlur.toFixed(2) },
    { key: 'mouseNoise', name: 'rim noise', cst: 'CONFIG.mouseNoise',
      min: 0, max: 1.2, step: 0.01, value: CONFIG.mouseNoise,
      text: () => CONFIG.mouseNoise.toFixed(2) },
