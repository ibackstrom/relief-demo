# PTSVer28 — corner particle cloud, under a label

ver28 is **ver18 with the mass moved**: 200 px right and 150 px up from where ver18 put it.
`offsetX` 0.150 → 0.391 and `offsetY` 0.230 → 0.411, both in viewport heights — 200 px and
150 px at a window 830 px tall, and proportional rather than fixed, so the move holds its
share of the frame at other window sizes instead of staying 200 px on a screen twice as big.
`?x=` and `?y=` take them live if that reference height is wrong for your screen.
