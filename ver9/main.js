// immersive-g.com home bas-relief reveal — ver9.
// ver8 at its WIDE feather — the size the customer picked — with the two changes they
// then asked for:
//   1. no "active / neon glow on the very tops of the relief": the fresnel rim, which
//      ver7 and ver8 inherited from the site at opacity 0.98, is the bright saturated
//      hairline sitting on the ridge edge. It is dropped to CHROMATIC.fresnelOpacity
//      below, so the contour is still marked but no longer glows against the halo.
//   2. the reveal zone halved: CONFIG.flowmap.falloff and FLUID.splatRadius together
//      (REVEAL_ZONE below). ?zone=full restores ver8's size for comparison.
...
// ---------------------------------------------------------------- reveal zone
// How big the revealed patch under the cursor is, as a multiple of ver8's. The customer
// asked for it "about 2x smaller", so 0.5 is the RADIUS of the patch, not its area — the
// patch you see is a disc, so this is half the width and a quarter of the area.
//   ?zone=full  ver8's size, for comparison
//   ?zone=0.7   any factor, e.g. 0.707 = half the AREA if half the width reads as too small
// It scales two things that must stay in step: the reveal brush (CONFIG.flowmap.falloff,
// a uv radius) and the dye splat that carries the colour (FLUID.splatRadius, which is in
// SQUARED distance — hence the zone factor squared). Let them drift apart and you get the
// old failure: a red disc smaller than the grey one, so every stroke keeps a grey rim.
//
// ?mask=site keeps the site's own brush unless a zone is asked for outright: that switch
// exists to show the untouched original, and a half-size brush is ours, not theirs.
const REVEAL_ZONE = PARAMS.get('zone') === 'full' ? 1
                  : parseFloat(PARAMS.get('zone')) > 0 ? parseFloat(PARAMS.get('zone'))
                  : PARAMS.get('mask') === 'site' ? 1
                  : 0.5;

// Shrinking the brush also makes the relief come out SHALLOWER, which is not what was
// asked for and is easy to miss: the flow-map accumulates the stamp frame by frame, so
// how deep a point ends up is (stamp strength x how many frames it spent under the
// brush). Halve the radius at the same cursor speed and every point spends half as long
// under it, so the same stroke reveals half as much relief and the colour — which needs
// extrude past ~0.4 before the deep bake levels mix in — mostly stops appearing.
// Measured: at 0.5 with no compensation the frame carried 0.09% colour against ver8's
// 1.37%, a 15x drop where the area alone accounts for 4x.
// So the deposit rate is scaled by 1/zone, restoring the depth per stroke and leaving
// the SIZE of the patch as the only thing that changed. ?ga=<n> overrides it.
...
const CHROMATIC = {
  ...
  base: 0,                  // no wash. The site has no such term; ver6 added one and
                            //   the trail read as a pink disc. "Slightly beyond the
                            //   contour" is the crease term's job, not a floor's.
...
const MASK = SITE_MASK ? { ...CHROMATIC, ...CHROMATIC_SITE }
                       : { ...CHROMATIC, ...FEATHER[FEATHER_SIZE] };
...
const FLUID = {
  ...
  splatRadius: 0.020 * REVEAL_ZONE * REVEAL_ZONE,
                            // dye blob size, in the sim's SQUARED-distance units.
...
if (!SITE_MASK) {
  ...
  document.title = 'ver9 — feather: ' + sizeName + ', rim: ' + MASK.fresnelOpacity
                 + ', zone: ' + REVEAL_ZONE;
  const hint = document.getElementById('hint');
  if (hint) hint.textContent = 'move the cursor over the wall — scroll to zoom';
}
