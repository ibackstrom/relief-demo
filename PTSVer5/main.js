...
function buildParticles(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // THROW AND TEST, in two dimensions. A point is thrown at the frame, the maps are
    // read where it lands, and it is kept if there is surface under it — no walk over the
    // geometry, no per-triangle bookkeeping. What makes this correct rather than merely
    // cheap is that the throw is uniform over the frame and the test is a plain accept:
    // whatever survives is uniformly dense over the model's PROJECTED area, which is the
    // area the viewer sees. A scatter weighted by surface area instead would crowd the
    // motes wherever the surface turns away from the camera, which is the rim — and the
    // rim is exactly where the eye reads the silhouette.
    //
    // The retry count is bounded and the last throw is taken wherever it fell. A mote
    // stranded in the background is one mote in thirty thousand, and insisting on a hit
    // makes the cost unbounded as the mask gets tighter.
    ...
    let px = (u - 0.5) * planeW;
    let py = -(w - 0.5) * planeH;
    let pz = (mapDepth - 0.5) * depthSpan;

    // A share of the motes ignores the maps completely and takes its own seat. This is the
    // reference's SECOND particle system: alongside the 40000 it masks to the portrait it
    // runs 8000 more with both thresholds at zero, which is to say unmasked, drifting hard
    // and displaced deeper. Without them the mask's edge is the edge of the effect, and a
    // mask edge is a cut line.
    //
    // Theirs are spread flat across the frame, where a rectangle has nothing to give away
    // because it IS the frame. Ours would show one, so the loose motes are drawn from a
    // Gaussian about the centre of the mass instead: density falls off smoothly forever
    // and there is no boundary anywhere to find.
