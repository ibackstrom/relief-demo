...
const CONFIG = {
  ...
  // ver18: 0.20 -> 0.28 - enough irregularity that the stretched mass has no clean oval
  seedLobes: 0.28,          // ver17: 0.42 -> 0.20 - irregular, but held together          // how far the lobes push the radius in and out. 0 is a circle
  // ver18: 1.15 -> 1.55 - the mass lies ALONG the title, behind the words
  seedAspectX: 1.30,        // ver17: 1.45 -> 1.15        // stretch along the row ...
  // ver18: 0.92 -> 0.70 - and thinner across it, so it stays behind the line of text
  seedAspectY: 0.70,        // ver17: 0.85 -> 0.92        // ... and across it
  ...
  // ver18: 0.18 -> 0.12 - ver30's pattern, smaller.
  // ver18: 0.12 -> 0.09. With the pull no longer squeezing the mass, its size comes from the
  // throw, so the throw is what gets smaller.
  cornerRadius: 0.09,       // AURORA ver11: back to ver10's pair (with cornerBias below) —
  ...
  // ver13: 0.118 -> 0.046, and this is a correction as much as a taste. fieldVelocity
  // returns v * (uSpeed * uFrequency) — the frequency is IN the velocity — so ver12's
  // 1.2 -> 2.6 on simFrequency did not only shrink the eddies, it more than doubled the
  ...
  // ver18: 0.118 -> 0.079, so the field's speed (uSpeed * uFrequency) stays at ver30's
  // 0.142 - the frequency is inside the velocity and would otherwise speed it up by half.
  simSpeed: 0.079,          // AURORA ver11: back to ver10's pace — the customer asked for
  ...
  // ver18: 1.2 -> 1.8. The field is in world units, so a smaller mass under ver30's own
  // frequency would see eddies bigger than itself and sway as one body instead of folding.
  // Scaled with the radius (1.2 * 0.18 / 0.12), the folds keep ver30's size RELATIVE to
  // the mass - the same pattern, smaller.
  simFrequency: 1.8,        // eddy size, as 1/frequency in world units. LOW on purpose: this
  ...
  // ver14: THE SWITCH IS A FLIGHT. Every mote is given a route from where it is to the same
  // place relative to the NEW tab, and flies it on its own schedule: its own start, its own
  // duration, its own bow. Summed over the flight each mote moves by exactly the distance
  // between the two tabs, so the mass arrives with the shape and the centring it left with -
  // under the tab, not short of it on whichever side it came from, which is where ver13's
  // carry dropped it.
  //
  ...
  centreLoop: false,
  // ver18: how far BELOW the tab's centre the mass is placed, in CSS px. The mass itself sits
  // centred on the tab, but what is SEEN of it weighs high: the menu is inside the page-bottom
  // fade, so the half of the cloud below the words is dimmed, and the risers drift upward.
  // Measured, the visible ink sat 6-15 px above the tab; this lowers the pull and the
  // reservoir together by the average of that, so the seen mass is on the line of the title.
  inkDropPx: 5,
  ...
  attractStagger: 0.70,     // AURORA ver13: how far the pull's strength is spread ACROSS the
                            ...
                            //   At 0.70 the eagerest mote is pulled nearly six times harder
  ...
  // ver18: 0.72 -> 0.45. The pull only has to keep the cloud on its tab; the flight carries
  // it between them. Weaker, it gathers without gathering INTO somewhere.
  // ver18: the pull only cancels drift now; the SEATS give the shape. A firm pull round a soft
  // middle herds the mass INTO that middle - measured, it put a third of the ink within 12 px
  // of the centre, 1.7x an even cloud of the same size - so it is kept weak.
  attractPull: 0.30,        // ver18: 0.45 -> 0.60 with the core sized to the mass,        // AURORA ver11: was 0.85 — softened so the gathering keeps its
  ...
  // ver18: 0.30 -> 0.55 - the soft middle made wider, so the part of the mass that is
  // left to the field is the body of the cloud, not only its very centre.
  // ver18: 0.55 -> 0.10. The soft core is measured against the pull's REACH, which is far
  // larger than the cloud - at 0.55 the whole mass sat inside it, nothing held its edges, and
  // it spread to 48 px across. At 0.10 it is soft only through the middle of the mass: no
  // sink at the centre, and a firm rim that keeps the cloud small.
  // ver18: back up to 0.45 with the weak pull: nothing is herded when nothing grips hard.
  attractCore: 0.45,        // as a fraction of the reach. INSIDE this the pull eases off,
  ...
  // ver14: 0.18 -> 0.80, and this one is MEASURED rather than judged. The grip's soft core
  // was a dead zone: inside it a mote felt almost no pull, so the field's down-left current
  // parked the settled mass off the tab and nothing corrected it - the ink's centre sat 12 to
  // 20 px left of the pressed tab, and further after a rightward switch. The ink centroid
  // against the tab centre, measured headless: 0.18 -> -13/-18/-12 px, 0.45 -> -2/-14/-6,
  ...
  // ver18: 1.0 -> 0.12. At full grip the pull stayed full-strength right into the tab's
  // centre, which makes that centre a SINK: every mote is drawn to one point and the mass
  // gathers into a knot there. With the soft core back, a mote near the middle is barely
  // pulled, so the ink spreads over the whole ground behind the title instead of pooling.
  attractCenterGrip: 0.12,   // AURORA ver11: the grip's floor AT the centre. The smoothstep
...
if (numParam('fly', 0.1, 3) !== null) CONFIG.flightDur = numParam('fly', 0.1, 3);
...
if (numParam('drop', -60, 60) !== null) CONFIG.inkDropPx = numParam('drop', -60, 60);
...
uniform float uFlightT0[8];      // when it began, in REAL seconds
uniform float uNow;              // real seconds now, and one frame ago
uniform float uNowPrev;
uniform float uClockScale;       // the sim clock's rate against real time, to date a birth
...
float flightEase(float t){ t = clamp(t, 0.0, 1.0); return t * t * t * (t * (t * 6.0 - 15.0) + 10.0); }
...
vec2 flightOwed(float seedw, float now, float birth){
  ...
  for (int k = 0; k < 8; k++) {
    vec2 D = uFlightD[k];
    if (dot(D, D) < 1e-14) continue;
    // born after this flight began: it was seated at the destination and owes nothing
    if (birth > uFlightT0[k]) continue;
...
// ver13 takes one argument more: the mote's own share of the pull. Every mote felt exactly
// the same force before, which is precisely what a TRANSFORM is — a rigid body carried from
// one place to another. Spread that force across the population and the same journey becomes
// a stream: the eager ones leave first and arrive first, the rest follow in their own time,
// and the mass stretches out over the row and gathers again rather than sliding along it.
// It costs nothing — the spread is the seed each mote already carries.
vec3 attractTo(vec3 p, vec3 target, float eagerness){
...
void main(){
  ...
  // ver14: THE FLIGHT. Each frame a mote is moved by however much its flights stopped owing it
  // since the last frame. Summed over a flight that is exactly the journey, so nothing can
  // land short or long, and because it is a displacement and not a force it takes the time it
  // says in real seconds, from the frame of the click. The velocity is left alone: the field
  // and the cursor go on working on a mote in flight, so it arrives still behaving like part
...
void main(){
  ...
  // seed.w is this mote's own lifespan — a per-mote constant, already in hand, and unrelated
  // to anything else the pull reads, so using its fraction here costs no texture and no
  // attribute. mix() keeps the population's AVERAGE pull at 1: the cloud as a whole is
  ...
  // ver14: aimed at where this mote's flight says it should be NOW, not at the tab. The flight
  // does the travelling; the pull only keeps the mote in formation around its own route, so
  ...
  // ver14: the real second of birth rides in w. It used to be inferred from the sim's age,
  // but that clock is capped per frame, so on a slow device a mote born BEFORE a switch could
  // look as if it was born after it - and would be left behind at the old tab.
  float bornAt = (age >= seed.w) ? uNow : birthV;
  gl_FragColor = vec4(v, bornAt);
}`);
  ...
  function sampleSeat() {
    ...
    if (CONFIG.cornerSeed) {
      ...
      if (CONFIG.cornerFull) {
        // ver18: the throw's own centre point, gone. A radius drawn as |gauss| at a uniform
        // angle spreads the same number of seats over every ring, and the rings near the middle
        // are tiny - so the density climbs as 1/r and spikes AT the centre. In ver30 that spike
        // sits on the screen corner and reads as ink spilling out of it; under a tab in the
        // middle of the screen it is the point the whole cloud seems to pour into. A Rayleigh
        // radius is a true 2D Gaussian - smooth and flat across the middle - scaled by 0.64 so
        // the mean radius, and so the size, stays what it was.
        rad = Math.sqrt(-2 * Math.log(1 - Math.random())) * cornerRadiusWorld
            * CONFIG.cornerBias * 0.64;
...
function makeSim() {
  ...
  const step = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uNow: { value: 0 },
      uNowPrev: { value: 0 },
      uClockScale: { value: CONFIG.speed },
...
function stepSim(dt) {
  ...
  // ver18: no 0.24 floor. It silently overrode any smaller soft core, and the soft core has to
  // be sized to the MASS, not to the pull's reach - see attractCore.
  u.uAttractCore.value = Math.max(CONFIG.attractCore / pullBoost, 0.02);
...
let travelBoost = 1;        // sim field speed multiplier while a flight is under way
const seatShift = new THREE.Vector2();   // AURORA ver10: seats' texture-space relocation

// ---- ver14: the flights ------------------------------------------------------------------
// A switch is a displacement handed to every mote to fly on its own schedule (see
// flightOwed in the sim). These are the bookkeeping: up to eight flights in the air at once,
// each a displacement and the real second it began. A slot is freed only once every mote
// has finished it, so a flurry of clicks is delivered in full rather than cut short.
const FLIGHT_SLOTS = 8;
const flightD = Array.from({ length: FLIGHT_SLOTS }, () => new THREE.Vector2());
const flightT0 = new Array(FLIGHT_SLOTS).fill(-1e6);
const flightJump = new THREE.Vector2();
let flightNow = 0;          // REAL seconds, not the sim's clock - see CONFIG.flightDur
let flightClockStart = null;
let flightPrev = 0;
let flightBusy = 0;         // 0..1, eased: something is in the air

// Where the cloud is settling, in the group's own units. It moves the moment a tab is
// pressed; the motes then fly to it. Set on the first read of the anchor and never lost.
let committed = null;
...
// AURORA: the pull's target element is re-read every frame. ver14: it no longer glides - the
// CSS transition is gone - so a click is a single jump of the anchor, and that jump becomes a
// flight. Small moves (a scroll, a resize) are followed at once instead of flown.
function updateAttract(vh, dt) {
  if (!sim) return;
  const r = readAnchor(vh);
  // The WALL clock, not an accumulation of frame steps. The frame step is capped (a stalled
  // frame must not blow the integrators up), so summing it runs slow whenever frames are
  // slow - a flight would stretch on exactly the devices that are already struggling. The
  // flight is a displacement, so a big step is still exact: it just lands in fewer frames.
  const nowS = performance.now() / 1000;
  if (flightClockStart === null) flightClockStart = nowS;
  flightPrev = flightNow;
  flightNow = nowS - flightClockStart;
  const dtR = Math.max(0, flightNow - flightPrev);
  flightJump.set(0, 0);
  if (r) {
    const ms = Math.max(1e-6, CONFIG.massScale);
    const dx = anchorLocal.x - committed.x;
    const dy = anchorLocal.y - committed.y;
    const dist = Math.hypot(dx, dy);
    const flyAt = (CONFIG.flightMinPx / innerHeight) * vh / ms;
    // A move is a FLIGHT only if the page said a tab was pressed (data-flight counts clicks).
    // On the live site the menu also moves when the page scrolls, and a scroll flown like a
    // switch would leave the cloud trailing the menu by most of a second. Anything the page
    // did not declare is followed in the same frame.
    ...
    if (declared && dist > flyAt) {
      ...
      for (let k = 0; k < FLIGHT_SLOTS; k++) {
        if (flightD[k].lengthSq() === 0) { slot = k; break; }
        if (flightT0[k] < flightT0[slot]) slot = k;
      }
      flightD[slot].set(dx, dy);
      flightT0[slot] = flightNow;
      committed.x += dx; committed.y += dy;
    } else if (dist > 1e-7) {
      flightJump.set(dx, dy);
      committed.x += dx; committed.y += dy;
    }
    ...
    let busy = 0;
    for (let k = 0; k < FLIGHT_SLOTS; k++) {
      if (flightD[k].lengthSq() === 0) continue;
      if (flightNow - flightT0[k] > life) flightD[k].set(0, 0);
      else busy = 1;
    }
    ...
    uniforms.uSignPoint.value.copy(committed);
...
// cloud is not centred on its origin, so the reservoir sat off the tab by however far the
// throw's own middle is from zero - and the pull, aimed at the tab, and the respawns, fed
// from beside it, settled the mass somewhere between the two. The seat cloud's MEAN goes on
// the tab now, which is the point the pull is holding, so the two agree.
//
// Across it jumps rather than glides. Living motes are compensated through uSeatDelta and do
...
function glideSeats() {
  ...
  // ver18: centred vertically too - the seat cloud's MEAN sits on the tab's centre, not at
  // whatever height the throw first put it. ver17 kept the tuned vertical, which held only as
  // long as the throw's shape did; squashing it to lie along the title moved its middle, and
  // the mass sagged 12-28 px below the words. Behind the title means ON its line.
